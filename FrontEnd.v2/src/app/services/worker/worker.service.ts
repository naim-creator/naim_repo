...
export class WorkerService {
  ...
  public getWorkersListByProfession(pageNo: number, id: any, profession: string): Observable<any> {
    ...
    return this.http.get(this.link + "/worker/get/profession/" + id + "?" + params.toString(), {headers: head_obj});
  }
  ...
  public getWorkerTotal(id: any): Observable<any> {
    let token = sessionStorage.getItem('token') as string;
    let head_obj = new HttpHeaders().set("Authorization", "Bearer " + token);
    return this.http.get(this.link + "/worker/get/quantity/" + id + "?", {headers: head_obj});
  }
