...
export class CompanyService {
  ...
  public getCompanyById(id: any): Observable<any> {
    let token = sessionStorage.getItem('token') as string;
    let head_obj = new HttpHeaders().set("Authorization", "Bearer " + token);
    return this.http.get(this.link + "/company/get/" + id, {headers: head_obj});
  }

  public getCompanyByContactorID(id: any): Observable<any> {
    let token = sessionStorage.getItem('token') as string;
    let head_obj = new HttpHeaders().set("Authorization", "Bearer " + token);
    return this.http.get(this.link + "/company/get/contactor/" + id, {headers: head_obj});
  }
