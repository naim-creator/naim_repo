...
export class NewDevisRequestComponent implements OnInit {
  ...
  public saveDevisRequest(): void {
    if (this.worker.company?.id != "") {
      this.devisRequest.company = {id: String(this.worker.company?.id)}
    } else {
      this.devisRequest.company = {id: String(this.contactor.company?.id)}
    }
    this.devisRequestService.addDevisRequest(this.devisRequest).subscribe({
      error: (err) => {
        if (err.status === 200) {
          this.route.navigate(['contactor/request-devis'])
        }
      }
    })
  }
  ...
  ngOnInit() {
    ...
    Webflow.push(function () {
      (document.getElementById('solarPanelList') as HTMLElement).classList.add('w--open');
    });
  }
}
