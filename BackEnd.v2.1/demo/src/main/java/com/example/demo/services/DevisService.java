...
public class DevisService {
    ...
    public BigDecimal getTotalDevis(UUID id) {
        return devisRepository.findTotalDevisByCompanyId(id);
    }

    public DevisResponse getDevisByCompanyFiltered(UUID id, int page, int size, String filter) {
        Pageable pageable = PageRequest.of(page, size);
        Page<Devis> devis = devisRepository.findByDevisCompanyIdFiltered(id, filter, pageable);
        ...
        DevisResponse devisResponse = new DevisResponse();
        devisResponse.setContent(devisDtos);
        devisResponse.setPageNo(devis.getNumber());
        devisResponse.setPageSize(devis.getSize());
        devisResponse.setTotalElements(devis.getTotalElements());
        devisResponse.setTotalPages(devis.getTotalPages());
        devisResponse.setLast(devis.isLast());
        return devisResponse;
