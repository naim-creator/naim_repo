...
public interface WorkerRepository extends JpaRepository<Worker, UUID> {
    ...
    @Query("SELECT w FROM Worker  w WHERE w.company.id = :companyId AND (UPPER(w.profession) LIKE %:profession%)")
    Page<Worker> findWorkerByProfession(@Param("profession") String profession, @Param("companyId") UUID companyId, Pageable pageable);

    @Query("SELECT count(w) FROM Worker w WHERE w.company.id = :companyId")
    BigDecimal countWorkersByCompanyId(@Param("companyId") UUID companyId);
}
