...
public interface SupplierRepository extends JpaRepository<Supplier, UUID> {
    ...
    @Query("SELECT s FROM Supplier s WHERE s.company.id = :companyId AND (s.firstName LIKE %:search% OR s.lastName LIKE %:search% OR s.email like %:search% OR s.phone LIKE %:search% OR s.SupplierCompanyName LIKE %:search%)")
